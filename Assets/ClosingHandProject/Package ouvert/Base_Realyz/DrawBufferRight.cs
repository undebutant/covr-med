...
public class DrawBufferRight : MonoBehaviour
{
  ...
  {
    DrawBufferRight.glDrawBuffer(1027U);
    uint error = DrawBufferRight.glGetError();
    if ((int) error == 0 || !this.reportErrorOnce)
      return;
    Debug.Log((object) ("[GL Error] glDrawBuffer(GL_BACK_RIGHT);: " + (object) error));
